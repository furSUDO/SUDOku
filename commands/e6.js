...
module.exports = {
	name: 'e6',
	description: 'Displays an image from e621.net matching your query',
	cooldown: 2,
        ...
        if (guildSettings.nsfw === 1) {
            ...
            if (message.channel.nsfw === true) {
                ...
                async function getFile(q){//function to fetch player id from name
                    var url = `https://e621.net/posts.json?tags=${q}&limit=1&page=${i}`;
                ...
                message.reply(`this is not an NSFW channel`)
            }
        }else{
            message.reply("NSFW commands disabled in this server!\nPlease ask an admin to run "+'``'+guildSettings.prefix+"module enable nsfw"+'``')
        }
	},
};